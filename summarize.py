...
PODCAST_PROMPT = r"""你是一位专业的内容分析师，擅长从播客对话中提取关键信息。请按以下要求处理给定的字幕文本。
...
def summarize_one(transcript: str, title: str, template: str = "default") -> str:
    """总结单个视频
    template: default, podcast
    """
    cfg = _get_llm_client()

    # 播客模板用完整字幕（不截断），其他用 12000 字限制
    if template == "podcast":
    ...
    return _llm_call(
        cfg,
        system=system,
        user=PODCAST_PROMPT.format(title=title, text=transcript) if template == "podcast"
             else f"视频标题: {title}\n\n字幕内容:\n{transcript}",
        max_tokens=4096,
    )
